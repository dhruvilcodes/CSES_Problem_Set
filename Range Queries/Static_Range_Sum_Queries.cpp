...
ll build_tree(vector<ll> &tree, vector<ll> &a, ll si, ll ei, ll ci)
{
    ...
    ll mid = (si + ei) / 2;
    return tree[ci] = build_tree(tree, a, si, mid, 2 * ci + 1) + build_tree(tree, a, mid + 1, ei, 2 * ci + 2);
}
...
ll rangeMaxQuery(vector<ll> &tree, ll si, ll ei, ll l, ll r, ll ci)
{
    ...
    return rangeMaxQuery(tree, si, mid, l, r, 2 * ci + 1) + rangeMaxQuery(tree, mid+1, ei, l, r, 2 * ci + 2);
}
    vector<ll> tree(800001, 0);
void solve()
{
    vector<ll> a;
    ll n, m;
